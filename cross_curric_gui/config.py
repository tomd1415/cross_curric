...
class Config:
    ...
    DOCX_FILENAME = OUTPUT_FILENAME + ".docx"
    SUMMARY_OUTPUT_FILENAME = f"{YEAR_GROUP}_{SUBJECT1}_Summary"
    SUMMARY_HTML_FILENAME = SUMMARY_OUTPUT_FILENAME + ".html"
    SUMMARY_PDF_FILENAME = SUMMARY_OUTPUT_FILENAME + ".pdf"
    SUMMARY_DOCX_FILENAME = SUMMARY_OUTPUT_FILENAME + ".docx"
    
    @staticmethod
    def update(subject1, subject2, year_group):
        Config.SUBJECT1 = subject1
        Config.SUBJECT2 = subject2
        Config.YEAR_GROUP = year_group
        Config.OUTPUT_FILENAME = f"{year_group}_{subject1}_{subject2}"
        Config.HTML_FILENAME = Config.OUTPUT_FILENAME + ".html"
        Config.PDF_FILENAME = Config.OUTPUT_FILENAME + ".pdf"
        Config.DOCX_FILENAME = Config.OUTPUT_FILENAME + ".docx"
        Config.SUMMARY_OUTPUT_FILENAME = f"{year_group}_{subject1}_Summary"
        Config.SUMMARY_HTML_FILENAME = Config.SUMMARY_OUTPUT_FILENAME + ".html"
        Config.SUMMARY_PDF_FILENAME = Config.SUMMARY_OUTPUT_FILENAME + ".pdf"
        Config.SUMMARY_DOCX_FILENAME = Config.SUMMARY_OUTPUT_FILENAME + ".docx"
...
"""
    ...
    SUMMARY_PROMPT = """Include more examples of links in this document than in the documents you are processing, as this document covers all other subjects, not just one. Prioritise the cross-curricular links based on the following criteria (from highest to lowest priority):
1. The more subjects involved in the same link, the higher the priority.
2. The easier it is to implement, the higher the priority.
3. The more important it appears to be, the higher the priority.
4. The more interesting the links are, the higher priority they should be.

End with a short paragraph (longer than in the documents you are processing) explaining how more cross-curricular links could be made. The report should be about one to one and a half pages in total length. The HTML used to markup the text is not for use in a web page and therefore only needs the tags that are in the examples. No other tags should be used. Please produce this in a format as close to the documents you are processing as you can, but taking into account that more subjects are involved. You are to summarise all the subject combinations in the documents you are processing. Remember all spelling and grammar should be British English. Once you have created the document please double check that the HTML is correct and as I asked."""
